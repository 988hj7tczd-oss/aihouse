...
class KiloCodeAdapter(AgentAdapter):
    ...
    def __init__(self, config: Optional[dict] = None) -> None:
        config = config or {}
        self._process_name: str = config.get("process_name", "kilo")

    def _find_extensions(self) -> bool:
        for ext_dir in VSCODE_EXT_DIRS:
            if ext_dir.is_dir():
                for item in ext_dir.iterdir():
                    if "kilo" in item.name.lower():
                        return True
        return False
    ...
    def get_current_task(self) -> Optional[AgentTask]:
        ...
        return AgentTask(
            agent_name=self.name, agent_type=self.agent_type,
            task_id=f"kilo_{procs[0]['pid']}_{int(started_at.timestamp())}",
            description="Kilo Code 正在运行",
